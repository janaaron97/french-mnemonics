...
**Tap any word in a sentence** during a round — in the cloze prompt, in a marked sentence, or in the round summary — and a sheet gives its headword, pronunciation, meaning, level and review stage, with a button to hear it and one to add it to your library. It resolves through the same inflection index the rest of the app uses, so `suis` finds *être*, and an elision or hyphen gives back both halves: `j'ai` is *je* and *avoir*, `peut-être` is *pouvoir* and *être*.

The corpus is built from content words and leaves out the grammar that holds a sentence together — `je`, `que`, `des`, `sur` — which is exactly what gets tapped first, so about ninety of those carry a plain gloss instead, marked as grammar rather than something to study. Between the two, **97.1% of the words across all 5,500 example sentences resolve**; the rest are mostly irregular feminine adjectives (`bonne`, `belle`, `longue`), a few past participles, and proper nouns. A word with nothing to say is simply not tappable rather than offering a guess.

The sheet is rendered through a portal: inside the arena it would sit in that stacking context and the answer bar would paint over it. Nothing about it animates opacity either — an entrance animation that fails to run to completion would leave it invisible while still catching every tap, which is exactly what happened the first time.
