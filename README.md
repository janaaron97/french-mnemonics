...
- `echo_state` — one row per person: the points total (still stored in the `xp` column, since the app only renamed the concept), rounds, best streak, the discover cursor, the level range, and the sound toggle.
