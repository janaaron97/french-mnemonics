...
iOS silences web audio whenever the ringer switch is off — Web Audio and speech alike, with no error raised anywhere. `navigator.audioSession.type = 'playback'` is the supported fix and covers Safari 16.4 and later. Where that API is missing the app falls back to a looping, near-silent `<audio>` element, which moves the page from the ambient session into playback; the waveform has to be genuinely non-zero, since a muted or truly silent element stays ambient and changes nothing.
...
Three specific quirks are handled. Safari reports an empty voice list until it has loaded voices, so the list is cached and refreshed on `voiceschanged`. It drops an utterance queued in the same tick as `cancel()`, so a cancel is followed by a queue on the next turn — and only when something is genuinely speaking, so the very first call keeps its user gesture. And it only lets the *first* utterance begin from inside a gesture, so the first pointer or key event anywhere in the app spends one on a silent space; the real speech later in a round is then never the one being refused. Building the utterance is itself wrapped, because a voice object can go stale across a `voiceschanged` and be refused on assignment — which would otherwise throw out of `speak()` entirely.

**Progress → Sound check** tests the three pipelines separately — Web Audio, an `<audio>` element playing a real generated tone, and speech — because iOS can mute them independently and knowing which one is silent is most of the answer. Each reports its own result, and **Copy report** puts the state on the clipboard.
