...
The two platforms need different handling, and pretending otherwise would produce a button that does nothing. Chromium fires `beforeinstallprompt`, which the app holds onto and replays from an **Install** button. **Safari has no such API at all** — on iOS the only route is Share → Add to Home Screen, so there the app shows those steps instead of a button. Anything else (desktop Firefox, an in-app browser) gets a plain sentence saying where its own menu keeps the option. Both surfaces appear in one dismissible line on the Play lobby, since installing is a once-ever action that has to be findable, and permanently under **Install on this device** on Progress.
